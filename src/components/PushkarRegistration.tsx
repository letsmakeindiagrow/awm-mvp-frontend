...
const OTPDialog: React.FC<OTPDialogProps> = ({ isOpen, onClose, onVerify, type }) => {
  ...
  React.useEffect(() => {
    if (isOpen) {
      ...
      // In a real application, this would be sent to the user's email/phone
      console.log(`OTP for ${type}: ${newOTP}`);
      alert(`OTP for ${type}: ${newOTP}`); // For demonstration purposes
    }
  }, [isOpen, type]);
...
const PushkarRegistration: React.FC = () => {
  ...
  const handleInitialSubmit = async () => {
    try {
      // Simulate API call for initial form submission
      ...
      // After successful submission, move to OTP verification
  ...
  const handleFinalSubmit = async () => {
    try {
      // Simulate final API call with verified status
      const finalData = {
        ...formData,
        isVerified: true,
      };
...
export default PushkarRegistration;